/**
 * Validation and form handling for New Movie page.
...
const VALIDATION_RULES = {
    ...
    winner: {
        required: false, // Boolean, not required but must be defined
        validate: (value) => {
            // Winner is a checkbox, so it's always valid (true or false)
            return null;
        }
    }
};
...
const fieldValidationState = {
    ...
    studios: false,
    producers: false,
    winner: true // Checkbox is always valid
};
...
function validateField(fieldName, value, showEmptyError = false) {
    ...
    if (errorMessage !== null && errorMessage !== '') {
        ...
        field.classList.add('is-invalid');
        if (errorDiv) {
            // Show error message only if showEmptyError is true (on blur)
            if (showEmptyError) {
...
function validateAllFields() {
    ...
    fields.forEach(fieldName => {
        ...
        if (field) {
            ...
            validateField(fieldName, value, false); // Don't show empty errors on initial load
        }
    });
    updateSaveButton();
}
...
/**
 * Saves the movie by sending POST request to the API.
 */
async function saveMovie() {
    ...
    // Validate all fields one more time before saving (show errors if any)
    const fields = ['year', 'title', 'studios', 'producers', 'winner'];
    fields.forEach(fieldName => {
        const field = document.getElementById(fieldName);
        if (field) {
            const value = field.type === 'checkbox' ? field.checked : field.value;
            validateField(fieldName, value, true); // Show errors before saving
        }
    });
    updateSaveButton();
    if (!Object.values(fieldValidationState).every(valid => valid === true)) {
        return; // Still invalid, don't save
    }
    ...
    try {
        ...
        saveButton.innerHTML = '<i class="fas fa-save me-2"></i>Save Movie';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (yearField) {
        // Year field is readonly, validation happens when year is selected from picker
        // Also allow clicking on the field to open picker
    ...
    if (titleField) {
        titleField.addEventListener('blur', () => {
            validateField('title', titleField.value, true); // Show error on blur
            updateSaveButton();
        });
        titleField.addEventListener('input', () => {
            clearTimeout(titleField.validationTimeout);
            titleField.validationTimeout = setTimeout(() => {
                validateField('title', titleField.value, false); // Don't show empty error on input
                updateSaveButton();
            }, 500);
        });
    }
    ...
    if (studiosField) {
        studiosField.addEventListener('blur', () => {
            validateField('studios', studiosField.value, true); // Show error on blur
            updateSaveButton();
        });
        studiosField.addEventListener('input', () => {
            clearTimeout(studiosField.validationTimeout);
            studiosField.validationTimeout = setTimeout(() => {
                validateField('studios', studiosField.value, false); // Don't show empty error on input
                updateSaveButton();
            }, 500);
        });
    }
    ...
    if (producersField) {
        producersField.addEventListener('blur', () => {
            validateField('producers', producersField.value, true); // Show error on blur
            updateSaveButton();
        });
        producersField.addEventListener('input', () => {
            ...
            producersField.validationTimeout = setTimeout(() => {
                validateField('producers', producersField.value, false); // Don't show empty error on input
                updateSaveButton();
            }, 500);
        });
    }
    ...
    // Initial validation (all fields will be invalid initially)
    validateAllFields();
...
function initializeYearPicker() {
    ...
    // Set initial range to include current year
...
function openYearPicker() {
    ...
    // Get current year value if exists
    const currentValue = yearField.value;
    if (currentValue) {
        selectedYear = parseInt(currentValue);
        // Adjust range to include selected year
